...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    bool perlo_on  = song_sect > 3 && song_sect < 5 && iCoo.y >= PERLO_BLOCK_OFFSET;
    bool drums_on  = song_sect > 3 && iCoo.y >= DRUMS_BLOCK_OFFSET;
    if(flower_on) fragColor.xyz =       true ? animFlowerData(iCoo,FLOWER_BLOCK,MIDI,FEEDBACK):vec3(1);     
    else if(pong_on)   fragColor.xyz =  true ? animPongData(iCoo,PONG_BLOCK,MIDI,FEEDBACK)  : vec3(1,0,0);//
    else if(perhi_on)  fragColor.xyz =  true ? animPerhiData(iCoo,PERHI_BLOCK,MIDI,FEEDBACK) : vec3(0,1,0);//
