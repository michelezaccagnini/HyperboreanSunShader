...
vec3 getEnvelope(int id, int chan, int row, float dur, sampler2D midi, sampler2D feedb)
{
...
vec3 getEnvelopeDrums(ivec2 tex_coo, int pitch1, int pitch2, int chan, float dur, sampler2D midi, sampler2D feedb)
{
        ...
        return vec3(env, is_on, pitch1);
}

vec3 getEnvelopeBass(ivec2 tex_coo, int pitch, int chan, float dur, sampler2D midi, sampler2D feedb)
{
        vec4  prev_data = iFrame < 10 ? vec4(0) : texelFetch(feedb,tex_coo,0);
        float prev_env  = prev_data.x, 
                was_on  = prev_data.y;
...
vec3 animPongData(ivec2 tex_coo, ivec4 block, sampler2D midi, sampler2D text)
{
    ...
    else if(block_id == 2)
    {
        int id = tex_coo.x;
        int chan = id + chan_offset;
        return getEnvelope(id, chan, PONG_ENV_ROW, 3., midi, text);
    } 
...
HitInfo map(vec3 p, sampler2D BUF_A, sampler2D MIDI, sampler2D TEXTURE)
{
    HitInfo res;
    res.dist = MAX_DIST;
    int song_sect = getSongSection(MIDI);
...
vec3 normal( in vec3 pos, sampler2D buffer_a, sampler2D midi, sampler2D another_texture)
{
    const float ep = 0.01;
    vec2 e = vec2(1.0,-1.0)*0.05773;
    return normalize( e.xyy*map( pos + e.xyy*ep, buffer_a, midi, another_texture).dist + 
					  e.yyx*map( pos + e.yyx*ep, buffer_a, midi, another_texture).dist + 
					  e.yxy*map( pos + e.yxy*ep, buffer_a, midi, another_texture).dist + 
					  e.xxx*map( pos + e.xxx*ep, buffer_a, midi, another_texture).dist );
}
