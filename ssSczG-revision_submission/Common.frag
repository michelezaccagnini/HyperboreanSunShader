...
{
    int section = int(getCCval(50,0,midi)*127.);
    return section;
}
...
vec3 getPosFlower(int id, float env, sampler2D midi)
{
    ...
    pos = vec3(cos(longi),data.y*2.-1.+pow(env,1.),sin(longi))*4.;//*vec3(data.x,1,data.x)*4.+FULCRUM1;
...
vec3 animFlowerData(ivec2 tex_coo, ivec4 block, sampler2D midi, sampler2D text)
{
    ...
    else if(block_id == 1)
    {
        ...
        vec3 p = texelFetch(text, tex_coo - ivec2(0,sub_block),0).xyz;
        return mix(vec3(0), p , stretch_ind);
    }
    else if(block_id == 2)
    {
        //petals
    ...
    else if(block_id == 4) 
    {
        ...
        return getEnvelope(id, chan, FLOWER_ENV_ROW, 5., midi, text);
    }
}
...
vec3 animPongData(ivec2 tex_coo, ivec4 block, sampler2D midi, sampler2D text)
{
    ...
    else if(block_id == 2)
    {
        int id = tex_coo.x;
        int chan = id + chan_offset;
        return getEnvelope(id, chan, PONG_ENV_ROW, 5., midi, text);
    } 
